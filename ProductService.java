...
public class ProductService {
    ...
    public List<Product> getAllFilteredByAvailability(String availability) {
    	List<Product> allProducts=getAll();
    	
    	Map<Integer,Product> uniqueProducts=unstableList.stream()
    			.filter(Objects::nonNull)
    			.collect(Collectors.toMap(
    					Product::getId,p ->p,(existing,duplicate)->existing));
    	return uniqueProducts.values().stream()
    			.filter(p->availability==null || availability.isEmpty() ||
    			           availability.equalsIgnoreCase(p.getAvailability()))
    			.collect(Collectors.toList());
    					
    	}
    	
    	return allProducts.stream()
    			.filter(p -> availability.equalsIgnoreCase(p.getAvailability()))
    			.collect(Collectors.toList());
    }
